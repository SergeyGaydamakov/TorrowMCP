...
export class ResourceHandlers {
  ...
  async handleNoteResource(
    ...
    if (
      ...
      throw new ValidationError(
        `Invalid note URI format. Expected: torrow://note/{noteId} where {noteId} should be replaced with actual note ID (e.g., torrow://note/aae6203eb30ec9f2624061bd89b595f57). Got: ${uri}`
      );
    }
  ...
  async handleArchiveResource(
    ...
    if (
      ...
      throw new ValidationError(
        `Invalid archive URI format. Expected: torrow://archive/{archiveId} where {archiveId} should be replaced with actual archive ID. Got: ${uri}`
      );
    }
