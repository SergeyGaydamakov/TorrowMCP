...
- `TORROW_TOKEN` - токен для доступа к Torrow API (обязательно)
...
**Важно:** В HTTP режиме токен для подключения к Torrow API берется из переменных окружения сервера, а не из конфигурации клиента. Убедитесь, что переменная `TORROW_TOKEN` установлена на сервере перед запуском.
